...
function simulateRequest<T>(data: T, delay = 300): Promise<T> {
...
export async function fetchDashboardSummary(
  ...
  const { startDate, endDate, product, origins } = params;
...
export async function fetchProcessDetail(
  ...
  let rows = base.rows.filter(row => {
    const matchProduct = !params.product || row.product === params.product;
    const matchOrigin = !params.origins?.length || params.origins.includes(row.origin);
