...
WHERE
  (
    ...
    AND (
      `mo_mes_db`.`mo_stereo_precheck`.`is_dirty_detect_enabled` = false
    )
    AND (
      `mo_mes_db`.`mo_stereo_precheck`.`is_color_cast_detect_enabled` = false
    )
    AND (
      `mo_mes_db`.`mo_stereo_precheck`.`is_cod_detect_enabled` = false
    )
  )