...
export class QualityManagementService {
  ...
  async getMeasureDistanceData(start: string, end: string, distance: string, precisions: number[]) {
    interface Row {
      date: string;
    ...
    `;
      ...
      const rows = await this.prisma.$queryRawUnsafe<Row[]>(query, precision, start, end, distance);

      result[precision] = rows.map(row => {
        const totalCount = typeof row.total_count === 'bigint' ? Number(row.total_count) : row.total_count;
        const validCount = typeof row.valid_count === 'bigint' ? Number(row.valid_count) : row.valid_count;

        return {
          date: row.date.toString().slice(0, 10),
          count: totalCount,
          percent: totalCount === 0 ? 0 : Number(((validCount / totalCount) * 100).toFixed(2)),
        };
      });
    }
  ...
  async getTableData(params: {
    ...
    for (const o of newRes.data) {
      if (o.error_code != 0) {
        o.ng_reason = await this.update_ng_reason(stepNo, o.ng_reason);
      }
    }
    ...
    if (!cameraSN) { newRes.data = newRes.data.slice(1, -1) }
    return {
  ...
  async update_ng_reason(stepNo: String, ng_reason: String) {
    const parts = ng_reason.split(',').map(s => s.trim()).filter(Boolean);
  ...
  async getFieldComment(tableName: string) {
    ...
    return records.reduce((acc, { COLUMN_NAME, COLUMN_COMMENT }) => {
  ...
  async mergeRows(rows, tableName, mapFieldComment) {
        ...
        Object.entries(val).forEach(([key2, val2]) => {
          ...
          if (key2 in mapFieldComment) {
            ...
            if (key2 in configDict) {
              let record = 0
              if (val.stage !== undefined && val.stage !== null && val.stage != '-1') {
                prefix = `-${val.stage}`
                record = 1
              }
            ...
            if ((key2 in configDict && prefix !== '') || !(key2 in configDict)) {
              ...
              if (!(newKey in dict) && val2 !== '' && val2 !== null) {
                dict[newKey] = val2 != -6259853400000000000 ? `${val2}` : "异常值"
    ...
    return {
      data: mergeRows,
      columns: Object.entries(newMapFieldComment).map(([prop, { label, isShow }]) => ({
        prop,
        label,
        isShow
      }))
  ...
  async getErrorCodes(params: {
    ...
    // 动态条件
    const conditions: string[] = [];
    if (startTime) conditions.push(`\`${tableName}\`.start_time >= '${startTime}'`);
    ...
    // 并行执行 SQL
    const allRows: any =
      await Promise.all(sqlList.map(sql => {
        console.log(sql)
  ...
  async getErrorCodesV2(params: {
    ...
    for (const o of rows) {
      if (o.error_code != 0) {
        o.ng_reason = await this.update_ng_reason(stepNo, o.ng_reason);
        o.ng_reason.split(';').forEach(o => o in errorDict ? errorDict[o] += 1 : errorDict[o] = 1)
  ...
  async getFQCErrorCodes(params: {
    ...
    const stats = rows.map(r => {
      const count = Number(r.count);
      return {
        error_code: r.check_result !== null ? r.check_result.toString() : 'null',
        count,
        percent: total === 0 ? '0%' : ((count / total) * 100).toFixed(1) + '%',
      };
    });
