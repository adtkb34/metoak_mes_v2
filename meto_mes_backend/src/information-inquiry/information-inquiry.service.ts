  ...
  async getProductInfoV2(sn, flowCode_) {
    ...
    for (const element of sns) {
      ...
      if (curSn) {
        ...
        data["物料"] = data["物料"] || {}
        materials.map(row => {
          const category = row.category ? `${row.id}-${row.category}-${row.position}` : `${row.id}-没有命名`
          const categoryValue = row.material_batch_no ? row.material_batch_no : row.material_serial_no;
        ...
        if (workCode || flowCode_) {
          const flowCode = flowCode_ ? flowCode_ : await this.getProcessFlowByWorkOrder(workCode)
          if (flowCode) {
            const flowInfo = await this.getProcessStepByFlowCode(flowCode)
            for (const element of flowInfo) {
              const workStage = await this.getStepByStepCode(element.stage_code)
              ...
              if (stepTypeNo === STEP_NO.CALIB) {
                ...
                const subData: Array<Map<string, string>> = await this.prisma.$queryRawUnsafe(sql)
                const sqlColumns = `
                ...
                columns.forEach(column => {
                  columnCommentsMap.set(column.COLUMN_NAME, column.COLUMN_COMMENT || column.COLUMN_NAME);
                });
                ...
                data["双目标定"] = data["双目标定"] || []
                data["双目标定"].push(...transformedData.map(o => {
                ...
                if (model) {
                  for (const snField of ['sn', 'camera_sn', 'CameraSN', 'beam_sn']) {
                    const fieldExists = await this.hasColumn(tableName, snField)

                    if (fieldExists) {
                      let sql
                      if (stepTypeNo && await this.hasColumn(tableName, 'mo_process_step_production_result_id')) {
                        ...
                        sql = "SELECT * FROM " + tableName + " where " + snField + " = '" + curSn + "'"
                      }
                      console.log(sql)
                      const subData: Array<Map<string, string>> = await this.prisma.$queryRawUnsafe(sql)
                      const sqlColumns = `
                      ...
                      if (!Array.isArray(data[stepName]) && subData.length > 0) {
                        data[stepName] = []
                      }
                      if (subData.length > 0 && 'mo_process_step_production_result_id' in subData[0]) {
                        data[stepName].push(...(await this.f(tableName, subData, columns, columnCommentsMap)))
                      } else if (subData.length > 0) {
                        if (stepTypeNo === STEP_NO.FQC) {
                        data[stepName].push(...transformedData.map(o => {
                          data["性能测试"].push(...transformedData.map(o => {
                            o['不良原因'] = o['检测结果'] === 1 ? "SUCCESS" : '失败'
                            return moveFieldsToFront(o)
                          }))
                        }))
                        } else {
    ...
    await Promise.all(rows.map(async item => {
      ...
      for (const key in item) {
        if (item.hasOwnProperty(key)) {
          ...
          let newKey
          if (item.position != null && positions?.map(String).includes(item.position.toString())) {
            if (item.stage && stages?.map(String).includes(item.stage.toString())) {
          ...
          if (item[key] !== null) {
            item[key] = item[key] === -6259853400000000000 ? "异常值" : item[key]
...
function moveFieldsToFront(obj) {
  const frontColumns = ['id', '主键ID', '横梁SN', '横梁序列号', 'cameraSN', 'camera_sn', '添加时间', '工序开始时间', '检测时间', 'TimeStamp', '错误代码', '错误码', '不良原因', '工位编号', '工位号','站点']
  // 复制原对象
  ...
  for (const key of frontColumns) {
    ...
    for (const objKey in obj) {
      if (objKey.toLowerCase() === lowerKey) { // 比较时忽略大小写
